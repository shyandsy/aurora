...
func newReg() *core.Registry { r := core.NewRegistry(); core.RegisterBuiltins(r); return r }

// fakeStore 是 consoleStore 的内存替身,测 Console 不必起真 DB。
type fakeStore struct {
	rows   map[int64]entity.RuleRow
	next   int64
	policy map[string]map[string]string // scope → {level: action}
}

func newFakeStore() *fakeStore {
	return &fakeStore{rows: map[int64]entity.RuleRow{}, next: 1, policy: map[string]map[string]string{}}
}
...
func (f *fakeStore) decisions(scope string, limit int, beforeID int64) ([]entity.DecisionRow, error) {
	return nil, nil
}
func (f *fakeStore) listRows(scope string) ([]entity.RuleRow, error) {
	var out []entity.RuleRow
...
func (f *fakeStore) getRow(id int64) (*entity.RuleRow, error) {
...
func (f *fakeStore) upsertRow(row *entity.RuleRow) error {
	if row.ID == 0 {
		row.ID = f.next
		f.next++
	}
...
func newTestConsole() (*fakeStore, Console) {
	st := newFakeStore()
	return st, newConsole(newReg(), st)
}

// dtoCond 造一个 ConditionDTO(params 用内联 map)。
func dtoCond(t string, params any) dto.ConditionDTO { return dto.ConditionDTO{Type: t, Params: params} }

// ── scope 注册表:Scopes 反映注册 + 动作类型 + 保存校验 scope(挡幻影配置) ──
func TestConsole_ScopeRegistry(t *testing.T) {
	reg := newReg()
	reg.RegisterScope("register", "注册", []core.ActionDef{
		{Name: "allow", Label: "放行", Kind: core.ActionTerminal},
		{Name: "email_verify", Label: "要求邮件激活", Kind: core.ActionFriction},
	})
	...
	if k, ok := reg.ActionKindOf("register", "email_verify"); !ok || k != core.ActionFriction {
		t.Errorf("email_verify 应 friction,got %v %v", k, ok)
	}
	if k, _ := reg.ActionKindOf("register", "allow"); k != core.ActionTerminal {
		t.Errorf("allow 应 terminal,got %v", k)
	}
	reg.RegisterScope("register", "", []core.ActionDef{{Name: "captcha"}}) // 不给 kind → 默认 terminal
	if k, _ := reg.ActionKindOf("register", "captcha"); k != core.ActionTerminal {
		t.Errorf("未声明类型应默认 terminal,got %v", k)
	}

	// UpsertRule:注册过的 scope 放行,未注册的拒(幻影 scope)
	good := dto.RuleDTO{Scope: "register", Name: "r", RiskLevel: string(core.RiskHigh),
		Conditions: []dto.ConditionDTO{dtoCond("asn_hosting", nil)}}
	if _, err := svc.UpsertRule(good); err != nil {
		t.Fatalf("已注册 scope 的规则应能存: %v", err)
	}
	bad := dto.RuleDTO{Scope: "login", Name: "r2", RiskLevel: string(core.RiskHigh),
		Conditions: []dto.ConditionDTO{dtoCond("asn_hosting", nil)}}
	if _, err := svc.UpsertRule(bad); err == nil {
		t.Error("未注册 scope 的规则应被拒(幻影配置)")
	}
	// SetPolicy 同样校验 scope
	if err := svc.SetPolicy("login", map[string]string{string(core.RiskHigh): "block"}); err == nil {
...
func TestConsole_CRUD(t *testing.T) {
	...
	created, err := svc.UpsertRule(dto.RuleDTO{
		Scope: "register", Name: "kill go", RiskLevel: string(core.RiskCritical), Combine: core.CombineAnd, Enabled: true,
		Conditions: []dto.ConditionDTO{dtoCond("ua_match", map[string]any{"patterns": []string{"Go-http-client"}})},
	})
	if err != nil {
	...
	if len(list) != 1 || len(list[0].Conditions) != 1 || list[0].Conditions[0].Type != "ua_match" || list[0].RiskLevel != string(core.RiskCritical) {
		t.Fatalf("应列出 1 条 ua_match/critical,got %+v", list)
	}

	// 非法(无条件)被拦,不入库
	if _, err := svc.UpsertRule(dto.RuleDTO{Scope: "register", RiskLevel: string(core.RiskHigh)}); err == nil {
		t.Fatal("无条件应被校验拦截")
	}
	...
	// 更新已存在(改等级)
	created.RiskLevel = string(core.RiskHigh)
	if _, err := svc.UpsertRule(*created); err != nil {
		t.Fatalf("更新失败: %v", err)
	}
	if list, _ := svc.ListRules("register"); list[0].RiskLevel != string(core.RiskHigh) {
		t.Fatalf("更新未生效,got riskLevel=%s", list[0].RiskLevel)
	}

	// 更新不存在
	if _, err := svc.UpsertRule(dto.RuleDTO{ID: 999, Scope: "register", RiskLevel: string(core.RiskHigh), Conditions: []dto.ConditionDTO{dtoCond("asn_hosting", nil)}}); !errors.Is(err, ErrRuleNotFound) {
